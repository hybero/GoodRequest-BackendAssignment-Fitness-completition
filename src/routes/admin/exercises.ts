...
router.get('/', verifyRoles('ADMIN'), async (req: Request, res: Response) => {
	...
	if(req.query.page && Number(req.query.page) < 1) return res.status(400).json({ 'message': localize(req, 'Parameter page must be bigger than 0.') })

	if(req.query.limit && Number(req.query.limit) < 1) return res.status(400).json({ 'message': localize(req, 'Parameter limit must be bigger than 0.') })

	if(req.query.programID && Number(req.query.programID) < 1) return res.status(400).json({ 'message': localize(req, 'Parameter programID must be bigger than 0.') })

	if(req.query.search && String(req.query.search).length < 1) return res.status(400).json({ 'message': localize(req, 'Search string must be at least 1 character long.') })
	...
	return res.json({
		data: exercises,
		message: localize(req, 'List of exercises.')
	})
})

router.post('/', verifyRoles('ADMIN'), async (req: Request, res: Response) => {

	if(!req.body.difficulty || !req.body.name || !req.body.programID) return res.status(400).json({ 'message': localize(req, 'Difficulty, name and programID are required.') })

	if(!Object.values(EXERCISE_DIFFICULTY).includes(req.body.difficulty)) return res.status(400).json({ 'message': localize(req, 'Difficulty value is invalid.') })
	...
	return res.json({
		data: exercise,
		message: localize(req, 'Exercise created.')
	})
...
router.put('/:id?', verifyRoles('ADMIN'), async (req: Request, res: Response) => {
	...
	if(!exercise) return res.status(404).json({ 'message': localize(req, 'Exercise not found.') })
	...
	return res.json({
		data: exercise,
		message: localize(req, 'Exercise updated.')
	})
...
router.put('/:id?/program', verifyRoles('ADMIN'), async (req: Request, res: Response) => {
	...
	if(!req.body.name) return res.status(400).json({ 'message': localize(req, 'Name is required.') })
	...
	if(!program) return res.status(404).json({ 'message': localize(req, 'Program for exercise not found.') })
	...
	return res.json({
		data: program,
		message: localize(req, 'Program updated.')
	})
